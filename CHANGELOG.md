...
- DataPlane (Kong Gateway) default version has been bumped to `v3.5.0`.
  [#1372](https://github.com/Kong/gateway-operator/pull/1372)
- Add horizontal autoscaling for `DataPlane`s using its `scaling.horizontal` spec field.
  [#1281](https://github.com/Kong/gateway-operator/pull/1281)
- ControlPlane default version has been bumped to `v3.0.1`. ControlPlane supported
  minor `v2.12` has been bumped to `v2.12.2`.
  [#1257](https://github.com/Kong/gateway-operator/pull/1257)
- The `ControlPlane`s now use Gateway Discovery by default, with Service DNS Strategy.
  Additionally, the `DataPlane` readiness probe has been changed to `/status/ready`
  when the `DataPlane` is managed by a `Gateway`.
  [#1261](https://github.com/Kong/gateway-operator/pull/1261)
...
- Fixed a problem where the operator would not set the defaults to `PodTemplateSpec`
  ...
  [#1224](https://github.com/Kong/gateway-operator/pull/1224)
- If the Gateway controller is enabled, `DataPlane` and `ControlPlane` controllers
  get enabled as well.
  [#1242](https://github.com/Kong/gateway-operator/pull/1242)
- Fix applying the `PodTemplateSpec` patch so that it's not applied when the
  ...
  which specified a port name instead of a number (which is what's generated by the operator)
  it would never reconcile and the status conditions would never get the up to date
  `ObservedGeneration`.
  [#1238](https://github.com/Kong/gateway-operator/pull/1238)
- Fix manager RBAC permissions which prevented the operator from being able to
  ...
  [#1255](https://github.com/Kong/gateway-operator/pull/1255)
...
- The `ControlPlane` `election-id` has been changed so that every `ControlPlane`
  has its own `election-id`, based on the `ControlPlane` name. This prevents pods
  belonging to different `ControlPlane`s from competing for the same lease.
  [#1349](https://github.com/Kong/gateway-operator/pull/1349)
...
[v1.2.0-alpha.1]: https://github.com/kong/gateway-operator/compare/v1.1.0..v1.2.0-alpha.1
